...
# Set the path for DLLs (specifically on Windows)
if os.name == 'nt':  # 'nt' is the os.name for Windows
    script_dir = os.path.dirname(os.path.abspath(__file__))
    os.environ['PATH'] = script_dir + os.pathsep + os.environ['PATH']
    dll_names = ['libmpv-2.dll']
    dll_path = next((ctypes.util.find_library(name) for name in dll_names if ctypes.util.find_library(name)), None)

    if dll_path is None:
        raise OSError('Cannot find libmpv-2.dll in your system %PATH%. Consider adding the directory containing the DLL to the PATH environment variable, or place the DLL in the same directory as this script.')
    backend = ctypes.CDLL(dll_path)
    fs_enc = 'utf-8'

else:
    # Ensure locale is set correctly for libmpv on UNIX-like systems
    locale.setlocale(locale.LC_NUMERIC, 'C')  # libmpv requires LC_NUMERIC to be "C"
    lc, enc = locale.getlocale(locale.LC_NUMERIC)

    sofile = ctypes.util.find_library('mpv')
    if sofile is None:
        raise OSError("Cannot find libmpv in the usual places. Depending on your distro, you may need to install mpv-devel or mpv-libs package. If libmpv is installed but not found, ensure its path is in the system's library path.")
    backend = ctypes.CDLL(sofile)
